...
\mprset{sep=1em}
\makeatletter
\newskip \mpr@andskip \mpr@andskip 1ex plus 0.5fil minus 0.5ex
\makeatother
...
%
\begin{abstract}
Session types are a type formalism used to specify and verify communication protocols. They have been integrated, among other paradigms, in the $\pi$-calculus~\cite{honda93,takeuchihonda94,hondavasconcelos98} and a linear concurrent $\lambda$-calculus, called Good Variation (GV)~\cite{wadler15,lindleymorris15}.
%
...
In Classical Processes (CP), a process calculus with strong ties to classical linear logic~\cite{wadler14}, deadlock freedom is guaranteed by combining channel creation and parallel composition in the $\pi$-calculus under the same logical cut rule. Similarly, deadlock freedom in GV is guaranteed by combining channel creation and thread spawning under the same operation, called fork.
%
...
%We illustrate PGV and the encoding via Milner's Cyclic Scheduler~\cite{milner89}.
