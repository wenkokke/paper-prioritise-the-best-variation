...
%
In both CP and GV, deadlock freedom is achieved at the expense of expressivity as the only processes allowed are tree-structured. Dardha and Gay~\cite{dardhagay18} define Priority CP (PCP), which allows cyclic-structures processes and restores deadlock freedom by using of \emph{priorities}, in line with Kobayashi and Padovani~\cite{kobayashi06,padovani14}.
%
Following PCP, we present Priority GV (PGV), a~variant of GV which decouples channel creation from thread spawning. We restore deadlock freedom by using priorities, while allowing cyclic-structured processes. PGV also solves several problems in the original GV. We show that our type system is sound by proving subject reduction and progress. We define an encoding from PCP to PGV and prove that the encoding preserves typing and is sound and complete with respect to the operational semantics.
