\begin{proof}
  ...
  \item
    Otherwise, $\tm{L}$ is ready to act on some endpoint $\tmty{y}{S}$. Let $\tmty{y'}{\co{S}}$ be the dual endpoint of $\tm{y}$. The typing rules enforce the linear use of endpoints, so there must be a term $\tm{L'}\in\{\tm{M_1},\dots,\tm{M_m},\tm{N}\}$ which uses $\tm{y'}$. There are two cases:
    \begin{enumerate}
    \item
