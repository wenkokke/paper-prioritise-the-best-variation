...
Terms are typed by a judgement $\tseq[\cs{p}]{\ty{\Gamma}}{M}{T}$ stating that ``a term $\tm{M}$ has type $\ty{T}$ and an upper bound on its priority $\cs{p}$ under the typing environment $\ty{\Gamma}$''. Typing for the linear $\lambda$-calculus is standard. Linearity is ensured by splitting environments on branching rules, requiring that the environment in the variable rule consists of just the variable, and the environment in the constant and unit rules are empty. Constants $\tm{K}$ are typed using type schemas, and embedded using \LabTirName{T-Const} (mid of \cref{fig:pgv-typing}). The typing rules treat {all variables} as linear resources, even those of non-linear types such as $\ty{\tyunit}$, though they can easily be extended to allow values with unrestricted usage~\cite{wadler14}.
