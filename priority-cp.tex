...
The main change is the move away from a reduction strategy based on cut elimination using commuting conversions, and towards a reduction strategy using the structural congruence.

\paragraph*{Priorities, types, and environments}
Types are based on classical linear logic connectives, and are annotated with priorities $\cs{o}\in\mathbb{N}$.
...
The types $\ty{\tyone[\cs{o}]}$ and $\ty{\tybot[\cs{o}]}$ type the endpoints of a channel whose session has finished, and over which we send or receive a ping before closing the channel. These act as the units for $\ty{\tytens[\cs{o}]{A}{B}}$ and $\ty{\typarr[\cs{o}]{A}{B}}$.
