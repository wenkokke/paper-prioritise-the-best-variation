...
\begin{mathpar}
  ...
  \begin{array}{cc}
    ...
    &
    {\tm{\res{x}{x'}{(\ppar{\recv{a}{y}{P}}{Q})}}}
    \\ 
    &
    \rotatebox[origin=c]{270}{$\red$}
    \\
    &
...
Processes $\pcp{\tm{\send{x}{y}{P}}}$ and $\pcp{\tm{\recv{x}{y}{P}}}$ send or receive over channel $\pcp{\tm{x}}$ a value $\pcp{\tm{y}}$ and proceed as process $\pcp{\tm{P}}$. $\pcp{\tm{\close{x}{P}}}$ and $\pcp{\tm{\wait{x}{P}}}$ send and receive an empty value -- denoting the closure of channel $\pcp{\tm{x}}$, and continue as $\pcp{\tm{P}}$.

Processes $\pcp{\tm{\inl{x}{P}}}$ and $\pcp{\tm{\inr{x}{P}}}$ make a left and right choice, respectively and proceed as process $\pcp{\tm{P}}$. $\pcp{\tm{\offer{x}{P}{Q}}}$ dually offers both left and right branches, with continuations $\pcp{\tm{P}}$ and $\pcp{\tm{Q}}$. $\pcp{\tm{\absurd{x}}}$ denotes the empty offer.
...
\end{restatabletheorem}
\begin{proof}
    Our proof follows the same reasoning by Kobayashi~\cite{kobayashi06} used in the proof of deadlock freedom for closed processes (Theorem 2). The details can be found in~\Cref{prf:thm-pcp-closed-progress}.
\end{proof}
...
\subsubsection*{Example: Milner's Cyclic Scheduler}\label{sec:pcp-example}
We illustrate our translation by giving the PCP version of the cyclic scheduler in~\cref{sec:pgv-example}. We implement the scheduler as follows, using $\pcp{\tm{\prod_{i}P_i}}$ to denote the parallel composition of the processes $\pcp{\tm{P_i}}$, and $\pcp{\plug{P}{Q}}$ to denote the plugging of $\pcp{\tm{Q}}$ in the one-hole process context $\pcp{\tm{P}}$:
\[
  \usingnamespace{pcp}
  \begin{array}{lrlr}
    \tm{\sched}
  ...
  \end{array}
\]
The translation of $\pcp{\sched}$ is exactly $\pgv{sched}$ where all processes are child processes.
