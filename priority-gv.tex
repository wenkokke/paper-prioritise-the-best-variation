...
\begin{example}[Cyclic Structure]
  \[\tm{%
    \begin{array}{l}
      ...
      \letpair{y}{y'}{\new}{}
      \\
      \andthen{\spawn\;(\andthen{\recv\;{x'}}{\send\;{\pair{\unit}{y}}})}{}
      \\
      \andthen{\send\;{\pair{\unit}{x}}}{\recv\;{y'}}
    \end{array}
  }\]
\end{example}
\end{minipage}%
...
\begin{example}[Deadlock]
  \[\tm{%
    \begin{array}{l}
      ...
      \andthen{\spawn\;(\andthen{\recv\;{x'}}{\send\;{\pair{\unit}{y}}})}{}
      \\
      \andthen{\recv\;{y'}}{\send\;{\pair{\unit}{x}}}
    \end{array}
  }\]
\end{example}
\end{minipage}
...
Firstly, we decompose the $\tm{\fork}$ construct.
...
In PGV, sequents are annotated with a priority bound $\cs{p}$, which is the \emph{upper bound} on the priority of actions performed while evaluating the term. The \emph{lower bound} is obtained from the typing environment. Terms with sequential evaluation, such as \LabTirName{T-Andthen}, have side conditions to ensure deadlock freedom. For instance, the side condition for typing $\tm{\andthen{M}{N}}$ requires that $\tm{M}$ has finished communicating before $\tm{N}$ starts communicating. The rule \LabTirName{T-Lam} stores the lower and upper priority bound of the function body in the type.
