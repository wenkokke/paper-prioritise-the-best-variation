...
\end{array}
\]
Duality plays a crucial role in session types. The two endpoints of a channel are assigned dual types, which ensures that, for instances, whenever one program \emph{sends} a value on a channel, the program on the other end is waiting to \emph{receive}. Each \emph{session type} $\ty{T}$ has a dual, written $\ty{\co{T}}$. Duality is an involutive function which preserves priorities.
\[
\begin{array}{lcl}
...
Let $\tm{x}$, $\tm{y}$, $\tm{z}$, and $\tm{w}$ to range over variable names. Occasionally, we use $\tm{a}$, $\tm{b}$, $\tm{c}$, and $\tm{d}$ to range over \emph{free} channel endpoints. The term language is the linear $\lambda$-calculus, extended with constants for the communication primitives.
\[
\begin{array}{lcl}
  \tm{L}, \tm{M}, \tm{N}
  & \coloneqq & \tm{x}
