...
\todo{The $\phi$ and $\child$ symbols are not explained at this stage and become clear only later. Either warn the reader or remove them altogether.}

\todo{An explanation is needed about what M and N represents and how the hole-construction models this "signal b when finished" behaviour.}

\todo{It is difficult to understand how a "close d" operation is supposed to work inside a (nu d d') construct without a corresponding wait d.}
...
A~diagram of our scheduler with three processes can be found in \Cref{fig:pgv-scheduler}.
\end{example}
...
\todo{Confusion arises from the difference between what is considered a native operator and a shortcut. For instance, "unit let" and "pair let" are native (representing sequence and new channel decomposition) while "singleton let" is a shortcut. The initial GV syntax was clear, this one is a bit confusing and might need some cleaning. At least explain why M;N cannot be a primitive construct.}
...
\todo{You might want to clarify (repeat) that external choice can be modeled by sum type communication only because the language uses linear channels.}
\todo{We also might want to emphasise that the modelling of choice in GV by~\cite{lindleymorris15} is unsound.}
...
\todo{This is a type-and-effect system and should probably be presented as such, as in [37].}
\Cref{fig:pgv-typing} gives the typing rules for PGV.
Typing rules for terms are at the top of \cref{fig:pgv-typing}. Terms are typed by a judgement $\tseq[\cs{p}]{\ty{\Gamma}}{M}{T}$ stating that ``a term $\tm{M}$ has type $\ty{T}$ and an upper bound on its priority $\cs{p}$ under the typing environment $\ty{\Gamma}$''. Typing for the linear $\lambda$-calculus is standard. Linearity is ensured by splitting environments on branching rules, requiring that the environment in the variable rule consists of just the variable, and the environment in the constant and unit rules are empty. Constants $\tm{K}$ are typed using type schemas, and embedded using \LabTirName{T-Const} (mid of \cref{fig:pgv-typing}).

\todo{From the way the typing rules \LabTirName{T-Var} and \LabTirName{T-Const} are formulated, it is clear that all variables are treated as linear resources, even if they happen to have type 1 or another non-linear type. This could be worth mentioning in the text, saying that the type system can be easily extended to ignore non-linear resources.}
...
In PGV, sequents are annotated with a priority bound $\cs{p}$, which is the \emph{upper bound} on the priority of actions performed while evaluating the term. The \emph{lower bound} is obtained from the typing environment. Terms with sequential evaluation, such as \LabTirName{T-LetUnit}, have side conditions to ensure deadlock freedom. For instance, the side condition for typing $\tm{\letunit{M}{N}}$ requires that $\tm{M}$ has finished communicating before $\tm{N}$ starts communicating. The rule \LabTirName{T-Lam} stores the lower and upper priority bound of the function body in the type.
